...
function parseLineToCommand(line) {
  // Skip empty lines
...
class YaparaState {
  constructor({
    ...
    this.readlineHandler = (line) => {
      const command = parseLineToCommand(line);
      if (!command) {
        return;
      }
      command.taskIndex = this.taskIndex;
  ...
  maybeSpawnTasks(outputHandler, tasks, maxConcurrent, terminationHandler) {
    while (this.commandsToStart.length > 0
      ...
      task.proc.on('error', (err) => {
        ...
        this.maybeSpawnTasks(outputHandler, tasks, maxConcurrent, terminationHandler);
      });
  ...
  maybeTerminate() {
    if (this.numRunningTasks === 0 && this.commandsToStart.length == 0 && this.readlineFinished) {
      outputHandler.finalize(this.exitStatus);
      process.exit(this.exitStatus);
    }
  }
}
