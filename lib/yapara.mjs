...
class Task {
  constructor({command, args, index, inputBuffer, name, parentState}) {
...
const DEFAULT_OPTIONS = {
  ...
  outputHandlers: [],
  rawOutputDir: null,
  rawStdoutFormat: '%06^t.out',
  rawStdoutCreateOnDemand: true,
  rawStderrFormat: null,
  rawStderrCreateOnDemand: true,
};

const DEFAULT_OUTPUT_FORMAT = 'colored';
...
function parseArguments() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (opt === 'help') {
        ...
        const handler = handlerType.make(options);
        options.outputHandlers.push(handler);
  ...
  if (options.rawOutputDir) {
    const handler = new OutputHandlerSeparateFiles(options);
    options.outputHandlers.push(handler);
  }

  if (options.outputHandlers.length === 0) {
    const handlerType = OUTPUT_HANDLER_TYPES[DEFAULT_OUTPUT_FORMAT];
    options.outputHandlers.push(handlerType.make(options));
  }
...
For outputting raw data:
  --raw-output-filename=FMT | out | err | null (default '%06^t.out')
  --raw-error-filename=FMT | out | err | null (default '%06^t.err')
  --raw-output-dir=DIR Save each task's output to separate files in this directory
...
function constructInputMapping({inputMode, commandPrefix}) {
  ...
  switch (mode) {
    case 'sh':
      return (line) => commandPrefix.concat(shell.parse(line, process.env));
    case 'argument':
      return (line) => commandPrefix.concat([line]);
    case 'tabsep':
      return (line) => commandPrefix.concat(line.split("\t"));
    default:
      console.error(`unhandled --input-mode ${mode}`);
      process.exit(1);
      break;
  }
}
